...
class DropBlock2D(nn.Module):
    ...
    def forward(self, input):
        ...
        Msum = F.conv2d(
            M,
            torch.ones(
                    (input.shape[1], 1, self.block_size, self.block_size)
                ).to(device=input.device,dtype=input.dtype),
            padding=self.block_size // 2,
            groups=input.shape[1])
        torch.set_printoptions(threshold=5000)
        mask = (Msum < 1).to(device=input.device, dtype=input.dtype)
        apply_the_mask = input * mask
        normal_the_feature = apply_the_mask * mask.numel() / mask.sum() #TODO input * mask * self.keep_prob ?
        return normal_the_feature
